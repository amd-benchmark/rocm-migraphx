...
DOCKERIMAGE=${DOCKERIMAGE:="rocm-migraphx:test"}
...
docker build --build-arg build_navi=${BUILD_NAVI} --build-arg base_image=${ROCM_BASE} --no-cache -t $DOCKERIMAGE -f dockerfile2 .